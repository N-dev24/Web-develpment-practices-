...
// Create new product 
export const createProduct: RequestHandler = (req: Request, res: Response): any => { 
  //Validate request 
  if (!req.body) { 
...
export const getAllProducts: RequestHandler = (req: Request, res: Response): any => { 
...
/// Get products by Id 
export const getProductById: RequestHandler = (req: Request, res: Response): any => { 
  Product.findByPk(req.params.id as any) 
  .then((data: Product | null) => { 
    return res.status(200).json({ 
      status: "success", 
      message: "Products successfully retrieved", 
...
///Modify product 
export const modifyProduct: RequestHandler = (req: Request, res: Response): any => { 
  // Validate request 
  if (!req.body) { 
    return res.status(400).json({ 
      status: "error", 
